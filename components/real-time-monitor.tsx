...
export default function RealTimeMonitor({ patient, thresholds }: RealTimeMonitorProps) {
  ...
  // Entrar na sala do paciente quando o componente montar
  useEffect(() => {
    if (socket && isConnected && patient.id) {
      socket.emit("join-patient-room", patient.id)
    }
  }, [socket, isConnected, patient.id])
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      sendVitalData(newVitals)

      // Emitir evento de atualização de sinais vitais via socket
  ...
  const sendVitalData = async (vitalData: typeof vitals) => {
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* SPO2 Card */}
        <Card className={`shadow-lg border-2 transition-all duration-500 ${getCardBackground("spo2", vitals.spo2)}`}>
          ...
          <CardContent>
            <div className="flex flex-col items-center justify-center py-6">
              <div
                ...
                style={{
                  borderColor: vitals.spo2 < thresholds.spo2.min ? "#f59e0b" : vitals.spo2 < 90 ? "#ef4444" : "#10b981",
                }}
              >
